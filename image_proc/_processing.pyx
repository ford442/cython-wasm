...
cpdef np.ndarray[np.uint8_t, ndim=3] process_tile(np.ndarray[np.uint8_t, ndim=3] tile):
    ...
    cdef np.ndarray[np.uint8_t, ndim=3] processed_tile
    cdef np.ndarray[np.uint8_t, ndim=3] dst, tile2,resize4x, result_1
    ...
    #del tile2
    resize4x = transform.rescale(dst, 2).astype(np.uint8)
    del dst
